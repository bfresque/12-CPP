...
/*   Updated: 2024/04/11 11:24:42 by bfresque         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void ScalarConverter::convert(const std::string &input) {
	...
	{
		char castC = input[0];
		int castI = static_cast<int>(castC);
		float castF = static_cast<float>(castC);
		double castD = static_cast<double>(castC);

		printChar(castC, input);
		printInt(castI, input);
		printFloat(castF, input);
		printDouble(castD, input);
	}
	else if (isChar(input) || isInt(input) || isFloat(input) || isDouble(input)) {
